...
static void build_vertices( OOInst_3DSolid *sol );
    ...
    switch (method)
        ...
        case OOM_NEW:
            if ((obj = (OObject*) OO_DoBaseMethod( cl, obj, method, attrlist )) != NULL)
                ...
                inst->Rot.Cx = inst->Rot.Cy = inst->Rot.Cz = inst->Rot.Ix = inst->Rot.Iy = inst->Rot.Iz = inst->Rot.SIx = inst->Rot.SIy = inst->Rot.SIz = (FLOAT)0;
                inst->Rot.Ax = inst->Rot.SAx = inst->Rot.OldSAx = OO3DMath_Deg2Rad((FLOAT)0);
                inst->Rot.Ay = inst->Rot.SAy = inst->Rot.OldSAy = OO3DMath_Deg2Rad((FLOAT)0);
                inst->Rot.Az = inst->Rot.SAz = inst->Rot.OldSAz = OO3DMath_Deg2Rad((FLOAT)0);
				...
				// Si changé dans SetGet_All()
				if (D3SOL(obj)->Flags & D3F_SOLID_TYPECHANGED)
					build_vertices( D3SOL(obj) );

				OOASSERT( inst->Type == NULL || inst->RotVertices != NULL);
                }
            return (ULONG)obj;
        ...
        case OOM_DELETE:
            ...
            OOPool_Free( OOGlobalPool, (UBYTE*)inst->RotVertices );
			OO3DType_RemoveType( inst->Type, FALSE );
        ...
        case OOM_SET:
            // Do not call base method
            {
			OORESULT rc = OOPrv_MSet( cl, obj, attrlist, OOF_MSET );
			// Si changé dans SetGet_All()
			if (D3SOL(obj)->Flags & D3F_SOLID_TYPECHANGED)
				build_vertices( D3SOL(obj) );
			return rc;
        ...
        //---------------
		// Appelé par objet Space
		case D3M_BUILDVERTICES:
			OOASSERT( D3SOL(obj)->RotVertices != NULL);
			build_vertices( D3SOL(obj) );
			return OOV_OK;
...
static void build_vertices( OOInst_3DSolid *sol )
{
	// Recopie des sommets dans coords de travail
	memcpy( sol->RotVertices, sol->Type->Vertices, sizeof(OO3DPoint)*sol->Type->NumVertices );
	// Rotation des sommets autour du centre de l'objet
	if (sol->Flags & D3F_DO_SELFROTATE)
		OO3DMath_RotateArray( sol->RotVertices, (OO3DPoint*)&sol->Rot.SAx, sol->Type->NumVertices );
	// Sommets en vraies coordonnées : ajout du centre de l'objet
	OO3DMath_AddArray( sol->RotVertices, (OO3DPoint*)&sol->Speed.nx, sol->Type->NumVertices );
}
...
static ULONG SetGet_All( OOSetGetParms *sgp )
{
    ...
    if (sgp->MethodFlag & (OOF_MSET|OOF_MNEW))
        ...
        switch (sgp->ExtAttr->ti_Tag)
            ...
            case D3A_RotIx      : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.Ix  != angle) { change = OOV_CHANGED; inst->Rot.Ix  = angle; } break;
            case D3A_RotIy      : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.Iy  != angle) { change = OOV_CHANGED; inst->Rot.Iy  = angle; } break;
            case D3A_RotIz      : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.Iz  != angle) { change = OOV_CHANGED; inst->Rot.Iz  = angle; } break;
            case D3A_RotAx      : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.Ax  != angle) { change = OOV_CHANGED; inst->Rot.Ax  = angle; } break;
            case D3A_RotAy      : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.Ay  != angle) { change = OOV_CHANGED; inst->Rot.Ay  = angle; } break;
            case D3A_RotAz      : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.Az  != angle) { change = OOV_CHANGED; inst->Rot.Az  = angle; } break;
            case D3A_RotSIx     : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.SIx != angle) { change = OOV_CHANGED; inst->Rot.SIx = angle; } break;
            case D3A_RotSIy     : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.SIy != angle) { change = OOV_CHANGED; inst->Rot.SIy = angle; } break;
            case D3A_RotSIz     : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.SIz != angle) { change = OOV_CHANGED; inst->Rot.SIz = angle; } break;
            case D3A_RotSAx     : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.SAx != angle) { change = OOV_CHANGED; inst->Rot.SAx = angle; } break;
            case D3A_RotSAy     : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.SAy != angle) { change = OOV_CHANGED; inst->Rot.SAy = angle; } break;
            case D3A_RotSAz     : angle = OO3DMath_Deg2Rad( flt ); if (inst->Rot.SAz != angle) { change = OOV_CHANGED; inst->Rot.SAz = angle; } break;
			...
			case OOA_Name: if (OOStr_IsEqual(inst->Name,sgp->ExtAttr->ti_String,FALSE) == FALSE) { change = OOV_CHANGED; inst->Name = sgp->ExtAttr->ti_String; } break;
			case D3A_Type:
				{
				OO3DType *remtype = NULL;
				OO3DType *newtype = NULL;
				if (sgp->ExtAttr->ti_Ptr == NULL)
					...
					if (inst->Type == NULL || OOStr_IsEqual( inst->Type->Node.Name, ((OO3DefType*)sgp->ExtAttr->ti_Ptr)->Name, FALSE ) == FALSE)
						{ 
						if ((newtype = OO3DType_AddType( sgp->ExtAttr->ti_Ptr )) != NULL)
							remtype = inst->Type;
						}
					}
				if (inst->Type != newtype)
					...
					if (newtype != NULL)
						{
						void *mem;
						if ((mem = OOPool_Resize( OOGlobalPool, inst->RotVertices, newtype->NumVertices*sizeof(OO3DPoint) )) == NULL)
							break;
						inst->RotVertices = mem;
						inst->Flags |= D3F_SOLID_TYPECHANGED;
						}
					change = OOV_CHANGED;
					}
				OO3DType_RemoveType( remtype, FALSE );
				inst->Type = newtype;
				}
				break;
			case D3A_TypeName: 
				if (inst->Type == NULL || OOStr_IsEqual(inst->Type->Node.Name,sgp->ExtAttr->ti_String,FALSE) == FALSE) 
					{
					OO3DType *type;
					if ((type = OO3DType_SearchType( sgp->ExtAttr->ti_String )) != NULL)
						{
						void *mem;
						if ((mem = OOPool_Resize( OOGlobalPool, inst->RotVertices, type->NumVertices*sizeof(OO3DPoint) )) == NULL)
							break;
						inst->RotVertices = mem;
						inst->Type = type;
						inst->Flags |= D3F_SOLID_TYPECHANGED;
        ...
        switch (sgp->ExtAttr->ti_Tag)
			...
			case D3A_TypeName: *(ULONG*)sgp->ExtAttr->ti_Ptr = (ULONG)(inst->Type == NULL ? NULL : inst->Type->Node.Name); break;
			case OOA_Name: *(ULONG*)sgp->ExtAttr->ti_Ptr = (ULONG)inst->Name; break;
            }
        }
    return change;
}
...
static OOAttrDesc attrs[] = {
    ...
    { D3A_Type			, (ULONG)NULL, "Type"			, SetGet_All    , OOF_MNEW | OOF_MSET |			   OOF_MNOT },
    { D3A_TypeName		, (ULONG)NULL, "TypeName"		, SetGet_All    , OOF_MNEW | OOF_MSET | OOF_MGET | OOF_MNOT },
    { OOA_Name			, (ULONG)NULL, "Name"			, SetGet_All	, OOF_MNEW | OOF_MSET | OOF_MGET | OOF_MNOT },
